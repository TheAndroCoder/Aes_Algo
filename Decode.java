public class Decode {
    ...
    public static KeySchedule keySchedule;

    //Inverse substitution box for inverse sub-bytes step
    ...
    public Decode(byte[][] s,KeySchedule k){
        this.stateArray=s;
        this.keySchedule=k;
    }
    ...
    /**
     * This method performs inverse sub-bytes step by replacing every byte of state array by the correspinding value in inverse s-box array
     */
    ...
    /**
     * This method performs inverse shift rows  by shifting each row 0,1,2,3 positions circularly to the right
     */
    ...
    /**
     * This method performs inverse mix columns operation on the state array
     */
    ...
    public static void invMatrixMul(int c) {
        //This is just representation of stateArray[*][c]
        byte a[]=new byte[4];
        for (int i = 0; i < 4; i++)
            a[i] = stateArray[i][c];
    ...
    /**
     * This method performs inverse add round key operation
     * @param round : round number of the inverse round key operation
     */
    public void invAddRoundKey(int round){
        keyArray=keySchedule.decodeNextKey(round);
        for (int column = 0; column <4 ; column++) {
            for (int row = 0; row <4 ; row++) {
