public class KeySchedule {
    ...
    //Round constant pre-calculated array (Rcon)
    ...
    //Constructor for generating expanded key
    public KeySchedule(byte[][] k){
        this.keyArray=k;

        //First four columns is same as the original key
        for (int i = 0; i <4 ; i++) {
            for (int j = 0; j <4 ; j++) {
    ...
    /**
     * Generates round key for round number :param
     * @param round : round number of the round key to be generated
     */
    public static void nextRoundKey(int round){
        ...
        //Make First column of the current round
        makeFirstColumn(prevFirstCol,prevLastCol,round);

        //Generate rest of the 3 remaining columns of the current round
    ...
    /**
     * This method makes the first column of the key and subsequently the other columns
     * @param prevFirstCol first column of the previous round key
     * @param prevLastCol last column of the previous round key
     * @param round round number of the current round
     */
    public static void makeFirstColumn(byte[] prevFirstCol,byte[] prevLastCol,int round){
        ...
        //Sub-bytes of rotword with s-box
        for (int k = 0; k <4 ; k++) {
            String hexStr = String.format("%x",rotWord[k]);
        ...
        //XOR first word of previous key & RowWord & Rcon
        for (int k = 0; k <4 ; k++) {
    ...
    /**
     * This method adds the generated round key to the expanded key array
     * @param round : round number of the generated round key
     */
    ...
    public byte[][] encodeNextKey(int round){
    ...
    /**
     * Decode class using the generated expanded key array
     * @param round current round number
     * @return returns round key for current round number
     */
    public byte[][] decodeNextKey(int round){
