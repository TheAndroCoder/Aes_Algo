public class Encode {
    ...
    //Holds the current block of key to apply
    public static byte[][] keyArray;
    //KeyShedule instance
    private static KeySchedule keySchedule;


    //pre-calculated substitution box lookup table
    ...
    //Constructor to encrypt this block of plain text with the key
    public Encode(byte[][] s,KeySchedule k){
        this.stateArray=s;
        this.keySchedule=k;
    }
    ...
    /**
     * Performs Substitute bytes operation on state array by replacing the byte with the value in substitution box
     */
    ...
    /**
     * Peforms shift rows operation on state array by circular left shift of each row by 0,1,2,3 bytes
     */
    public static void shiftRows(){
        int offset=0;
        byte[] originalRow = new byte[4];
        int position;
        for (int row = 0; row <4 ; row++) {
            originalRow[0]=stateArray[row][0];
            originalRow[1]=stateArray[row][1];
            originalRow[2]=stateArray[row][2];
            originalRow[3]=stateArray[row][3];
            //rotate the row in forward direction
            for (int column = 0; column <4 ; column++) {
                position=(column-offset)%4;
                if(Math.signum(position)==-1){
                    //substract from end if item is displaced from the front
                    position=4+position;
                }
                stateArray[row][position]=originalRow[column];
            }
            offset++;
        }
    ...
    /**
     * This method performs Mix Columns operation on the state array
     */
    public static void mixColumns(){
        //Multiply columns of state by mix-columns matrix
        for (int column = 0; column <4 ; column++) {
            matrixMul(column);
        }
    }
    ...
    /**
     * Performs matrix multiplication -> stateArray matrix * Rcon Array
     * @param c : Column number which is to be multiplied with Rcon Array
     */
    public static void matrixMul(int c){
        byte[] a=new byte[4]; //This array is just a copy of stateArray[*][c]
        for (int i = 0; i < 4; i++) {
            a[i]=stateArray[i][c];
        }
    ...
    /**
     * This method performs add round key operation on the state array
     * @param round : the round number of add round key operation
     */
    public static void addRoundKey(int round){
        //Retreives the next 4x4 key array
        keyArray=keySchedule.encodeNextKey(round);
        //XOR columns of state array and key
        for(int column = 0; column < 4; column++){
